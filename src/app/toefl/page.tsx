  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-4xl px-4 py-6">
        ...
        <h1 className="text-2xl font-bold">🎓 Modo TOEFL</h1>
        <p className="mb-6 text-sm text-slate-500">
          Aquí mides tu puntaje ITP estimado y presentas exámenes de nivel para
          desbloquear el siguiente. Es opcional: tu aprendizaje diario está en el
          panel.
        </p>
        ...
        <div className="card">
          <div className="mb-2 flex items-end justify-between">
            <div>
              ...
              <p className="text-4xl font-bold text-brand-600">{data.estimatedItpScore}</p>
          ...
          <div className="mt-3 h-3 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
            <div
              className={`h-full rounded-full transition-all ${reachedGoal ? "bg-emerald-500" : "bg-brand-500"}`}
              style={{ width: `${goalPct}%` }}
            />
          </div>
        ...
        <div className="mt-4 card bg-brand-600 text-white dark:bg-brand-700">
          <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
            <div>
              <h2 className="text-lg font-semibold">
                {data.levelLessonsCompleted
                  ...
                  : `Nivel ${data.currentLevel} — ${levelDef.name}`}
              </h2>
            ...
            {data.levelLessonsCompleted ? (
              <Link href={`/exam/${data.currentLevel}`} className="btn bg-white text-brand-700 hover:bg-brand-50">
                Tomar examen de nivel
              </Link>
            ) : data.continueLessonId ? (
              <Link href={`/lesson/${data.continueLessonId}`} className="btn bg-white text-brand-700 hover:bg-brand-50">
                Continuar lecciones
              </Link>
            ) : null}
          </div>
        ...
        <section className="mt-6">
          <h2 className="mb-3 text-lg font-semibold">Lecciones del nivel {data.currentLevel}</h2>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{AREA_LABELS[l.area]}</p>
                  </div>
                </div>
            ...
            <ol className="space-y-2">
              {LEVELS.map((lv) => {
                ...
                return (
                  <li
                    key={lv.level}
                    className={`flex items-center gap-3 rounded-lg px-2 py-1.5 ${current ? "bg-brand-50 dark:bg-brand-950" : ""}`}
                  >
                    <span className="text-lg">{unlocked ? (current ? "📍" : "✓") : "🔒"}</span>
                    <div>
                      <p className="text-sm font-medium">Nivel {lv.level} — {lv.name}</p>
                      <p className="text-xs text-slate-400">{lv.bandLabel} · {lv.cefr}</p>
                    </div>
                  </li>
                );
              })}
